...
void lcd_init()
{
   ...
   {
	   _lcd_select_area( 0, y, LCD_WIDTH - 1, y );
...
void lcd_put_pixel( uint16_t t_x, uint16_t t_y, uint16_t t_color )
{
    ...
    tx_buf[ 21 ] = 0;
    tx_buf[ 22 ] = (t_color >> 8) & 0xFFU;
    tx_buf[ 24 ] = t_color & 0xFFU;

    _spi_send_data( tx_buf,  sizeof( tx_buf ) );
}

void lcd_put_square( uint16_t t_x, uint16_t t_y, uint16_t t_color, uint8_t t_size_1_8 )
{
	t_size_1_8 = MAX( 1, MIN( 8, t_size_1_8 ) ); // range 1-8
	uint8_t tx_buf[ 22 + 4 * t_size_1_8 * t_size_1_8 ];
    memset( tx_buf, 0xFF, sizeof( tx_buf ) );
    ...
    for ( uint32_t inx = 22; inx < sizeof( tx_buf ); inx += 4 )
    {
        tx_buf[ inx + 0 ] = (t_color >> 8) & 0xFFU;
        tx_buf[ inx + 2 ] = t_color & 0xFFU;
    }
